...
import { Fetcher as FetcherSpirit, Token as TokenSpirit, ChainId } from '@spiritswap/sdk';
...
export class EctoFinance {
   ...
   */
  async geeBondStat(): Promise<TokenStat> {
  ...
  async getEctoPriceInLastTWAP(): Promise<BigNumber> {
    const { Treasury } = this.contracts;
  ...
  async earnedFromBank(
    ...
    try {
      if (earnTokenName === 'ECTO') {
        return await pool.pendingECTO(poolId, account);
      } else {
  ...
  async getMasonryAPR() {
    ...
    const ESHAREPrice = (await this.geeShareStat()).priceInDollars;
    const ECTOPrice = (await this.getEctoStat()).priceInDollars;
    const epochRewardsPerShare = lastRewardsReceived / 1e18;

    //Mgod formula
    const amountOfRewardsPerDay = epochRewardsPerShare * Number(ECTOPrice) * 4;
  ...
  async watchAssetInMetamask(assetName: string): Promise<boolean> {
    ...
    if (ethereum && ethereum.networkVersion === config.chainId.toString()) {
      ...
      if (assetName === 'ECTO') {
        ...
        asset = this.ESHARE;
        assetUrl = 'https://ecto.finance/presskit/eshare_icon_noBG.png';
      } else if (assetName === 'EBOND') {
        asset = this.EBOND;
        assetUrl = 'https://ecto.finance/presskit/ebond_icon_noBG.png';
      }
  ...
  async swapEBondToEShare(ebondAmount: BigNumber): Promise<TransactionResponse> {
    const { EShareSwapper } = this.contracts;
    return await EShareSwapper.swapEBondToEShare(ebondAmount);
  }
  async estimateAmountOfEShare(ebondAmount: string): Promise<string> {
    const { EShareSwapper } = this.contracts;
    try {
      const estimateBN = await EShareSwapper.estimateAmountOfEShare(parseUnits(ebondAmount, 18));
  ...
  async getEShareSwapperStat(address: string): Promise<EShareSwapperStat> {
    const { EShareSwapper } = this.contracts;
    const eshareBalanceBN = await EShareSwapper.getEShareBalance();
    const ebondBalanceBN = await EShareSwapper.getEBondBalance(address);
    // const ectoPriceBN = await EShareSwapper.getEctoPrice();
    // const esharePriceBN = await EShareSwapper.getESharePrice();
    const rateESharePerEctoBN = await EShareSwapper.getEShareAmountPerEcto();
    const eshareBalance = getDisplayBalance(eshareBalanceBN, 18, 5);
    const ebondBalance = getDisplayBalance(ebondBalanceBN, 18, 5);
    return {
      eshareBalance: eshareBalance.toString(),
      ebondBalance: ebondBalance.toString(),
      // ectoPrice: ectoPriceBN.toString(),
      // esharePrice: esharePriceBN.toString(),
