...
const Sbs: React.FC = () => {
  ...
  const [ebondAmount, setEbondAmount] = useState('');
  const [eshareAmount, setEshareAmount] = useState('');
  ...
  const eshareSwapperStat = useEShareSwapperStats(account);

  const eshareBalance = useMemo(
    () => (eshareSwapperStat ? Number(eshareSwapperStat.eshareBalance) : 0),
    [eshareSwapperStat],
  );
  const bondBalance = useMemo(
    () => (eshareSwapperStat ? Number(eshareSwapperStat.ebondBalance) : 0),
    [eshareSwapperStat],
  );
  ...
  const handleEBondChange = async (e: any) => {
    ...
    setEbondAmount(e.currentTarget.value);
  ...
  const handleEShareSelectMax = async () => {
    ...
    setEbondAmount(getDisplayBalance(updateEBondAmount, 18, 6));
  };
                      ...
                      <StyledApproveWrapper>
                        {approveStatus !== ApprovalState.APPROVED ? (
                          ...
                          <Button
                            color="primary"
                            variant="contained"
                            onClick={() => onSwapEShare(ebondAmount.toString())}
                            size="medium"
                          >
                            Swap
                          </Button>
