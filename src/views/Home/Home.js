...
import useBurnedESHARE from '../../hooks/useBurnedESHARE.js';
...
const Home = () => {
  ...
  const eBondStats = useBondStats();
  const ectoFinance = useEctoFinance();
  const { balance } = useBurnedESHARE();
  ...
  const eBondPriceInDollars = useMemo(
    () => (eBondStats ? Number(eBondStats.priceInDollars).toFixed(2) : null),
    [eBondStats],
  );
  const eBondPriceInFTM = useMemo(() => (eBondStats ? Number(eBondStats.tokenInFtm).toFixed(4) : null), [eBondStats]);
  const eBondCirculatingSupply = useMemo(
    () => (eBondStats ? String(eBondStats.circulatingSupply) : null),
    [eBondStats],
  );
  const eBondTotalSupply = useMemo(() => (eBondStats ? String(eBondStats.totalSupply) : null), [eBondStats]);

  const ectoLpZap = useZap({ depositTokenName: 'ECTO-FTM-LP' });
  const eshareLpZap = useZap({ depositTokenName: 'ESHARE-FTM-LP' });
  ...
  const [onPresentEshareZap, onDissmissEshareZap] = useModal(
    <ZapModal
      ...
      onConfirm={(zappingToken, tokenName, amount) => {
        if (Number(amount) <= 0 || isNaN(Number(amount))) return;
        eshareLpZap.onZap(zappingToken, tokenName, amount);
        onDissmissEshareZap();
      }}
        ...
        <Grid item xs={12} sm={4}>
          <Card>
            <CardContent align="center" style={{ position: 'relative' }}>
              ...
              Current Price
              <Box>
                <span style={{ fontSize: '30px' }}>{eBondPriceInFTM ? eBondPriceInFTM : '-.----'} FTM</span>
              </Box>
              <Box>
                <span style={{ fontSize: '16px' }}>${eBondPriceInDollars ? eBondPriceInDollars : '-.--'}</span>
              </Box>
              <span style={{ fontSize: '12px' }}>
                Market Cap: ${(eBondCirculatingSupply * eBondPriceInDollars).toFixed(2)} <br />
                Circulating Supply: {eBondCirculatingSupply} <br />
                Total Supply: {eBondTotalSupply}
              </span>
            </CardContent>
          </Card>
        </Grid>
        ...
        <Grid item xs={12} sm={6}>
          <Card>
            <CardContent align="center">
              ...
              <Box mt={2}>
                <Button color="primary" onClick={onPresentEshareZap} variant="contained">
                  Zap In
                </Button>
              </Box>
              <Box mt={2}>
                <span style={{ fontSize: '26px' }}>
                  {eshareLPStats?.tokenAmount ? eshareLPStats?.tokenAmount : '-.--'} ESHARE /{' '}
                  {eshareLPStats?.ftmAmount ? eshareLPStats?.ftmAmount : '-.--'} FTM
                </span>
              </Box>
              <Box>${eshareLPStats?.priceOfOne ? eshareLPStats.priceOfOne : '-.--'}</Box>
              <span style={{ fontSize: '12px' }}>
                Liquidity: ${eshareLPStats?.totalLiquidity ? eshareLPStats.totalLiquidity : '-.--'}
                <br />
                Total supply: {eshareLPStats?.totalSupply ? eshareLPStats.totalSupply : '-.--'}
              </span>
            </CardContent>
          </Card>
        </Grid>
